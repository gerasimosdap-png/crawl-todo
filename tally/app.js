...
const ICONS = [
  ...
  { kw: ['run', 'jog'], emoji: '🏃', tip: 'movement' },
  { kw: ['walk', 'steps', 'stroll'], emoji: '🚶', tip: 'movement' },
  { kw: ['yoga'], emoji: '🧘', tip: 'movement' },
  { kw: ['stretch', 'mobility'], emoji: '🤸', tip: 'movement' },
  { kw: ['gym', 'workout', 'lift', 'weight', 'exercise', 'train', 'pushup', 'plank'], emoji: '🏋️', tip: 'movement' },
  ...
  { kw: ['writ'], emoji: '✍️', tip: 'journal' },
  { kw: ['floss', 'brush', 'teeth', 'dental'], emoji: '🦷', tip: 'dental' },
  { kw: ['skincare', 'skin care', 'moisturi', 'serum'], emoji: '🧴', tip: 'selfcare' },
  ...
  { kw: ['fruit', 'veg', 'salad', 'greens'], emoji: '🥗', tip: 'nutrition' },
  { kw: ['laundry'], emoji: '🧺', tip: 'chores' },
  { kw: ['dishes', 'dishwash'], emoji: '🍽️', tip: 'chores' },
  { kw: ['clean', 'tidy', 'vacuum', 'hoover', 'declutter'], emoji: '🧹', tip: 'chores' },
  ...
  { kw: ['calisthenic', 'callisthenic'], emoji: '🏋️', tip: 'movement' },
  ...
  { kw: ['make bed', 'iron', 'ironing', 'wash', 'mop'], emoji: '🧼', tip: 'chores' },
  ...
  { kw: ['sunscreen', 'spf'], emoji: '🧴', tip: 'selfcare' },
  { kw: ['recipe', 'snack', 'smoothie'], emoji: '🍎', tip: 'nutrition' },
  { kw: ['pilates', 'calisthenic', 'callisthenic', 'hiit', 'crossfit', 'cardio', 'burpee', 'squat', 'lunge', 'situp', 'sit-up', 'press-up', 'treadmill', 'elliptical', 'rowing', 'sprint', 'marathon', 'parkrun', 'kettlebell', 'dumbbell', 'deadlift', 'gymnastics', 'sport', 'spin class'], emoji: '🏋️', tip: 'movement' },
  ...
  { kw: ['surf', 'paddleboard', 'kayak', 'canoe'], emoji: '🏄', tip: 'movement' },
  { kw: ['physio', 'rehab', 'stretching'], emoji: '🤸', tip: 'movement' },
  { kw: ['hobby', 'hobbies', 'pottery', 'ceramics', 'origami', 'calligraphy', 'scrapbook', 'woodwork', 'whittl', 'model kit', 'lego'], emoji: '🎨', tip: 'practice' },
  { kw: ['blog'], emoji: '✍️', tip: 'journal' },
  ...
  { kw: ['spa day', 'sauna', 'hot tub'], emoji: '🧖', tip: 'selfcare' },
  { kw: ['massage'], emoji: '💆', tip: 'selfcare' },
  { kw: ['relax', 'unwind', 'me time', 'self-care', 'rest day', 'wind down'], emoji: '🌿', tip: 'selfcare' },
...
function defaultState() {
  return {
    version: 1,
    tasks: [],
    settings: { theme: 'sage', notify: false, reminderTime: '20:00', name: '', lastNotifyDay: '', gcalConnected: false, gcalShow: true, gcalClientId: '', celebratedWeek: '', syncEnabled: false },
    onboarded: false,
    createdAt: Date.now(),
    updatedAt: Date.now()
  };
...
-------------------------------------------------------- */
function newTask(o) {
  return Object.assign({ id: uid(), title: '', type: 'daily', days: [], target: 3, completions: [], archived: false, createdAt: Date.now() }, o);
}
...
function renderDayCalendar(v, key) {
  ...
  if (gcalEventsByDay[key] === undefined) { gcalFetchDay(key); return; }
...
function weeklyRate() {
  ...
  S.tasks.forEach(t => {
    if (t.type === 'daily') {
      for (let i = 0; i < daysElapsed; i++) { expected++; if (compsOn(t, dateKey(wd[i])) > 0) done++; }
    } else if (t.type === 'weekdays') {
      for (let i = 0; i < daysElapsed; i++) { if ((t.days || []).includes(i)) { expected++; if (compsOn(t, dateKey(wd[i])) > 0) done++; } }
...
function saveEditor() {
  ...
  if (editingId) {
    ...
    Object.assign(t, { title: draft.title, type: draft.type, days: draft.days, target: draft.target });
    if (t.type === 'finite' && totalComps(t) < t.target) t.archived = false;
  } else {
    addTask({ title: draft.title, type: draft.type, days: draft.days, target: draft.target });
  }
...
async function gcalInitSilent() {
  try { await gcalInitClient(); const tok = await gcalEnsureToken(); if (tok) gcalFetchDay(activeDayKey()); } catch (e) {}
}
