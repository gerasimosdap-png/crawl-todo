...
function renderStats() {
  ...
  if (S.settings.aiEnabled) {
    ...
    ac.innerHTML = '<h3>Your coach</h3>';
    const ins = computeInsights();
    if (ins.length) {
      const list = el('div', 'insights');
      ins.forEach(s => { const r = el('div', 'insight'); r.textContent = s; list.appendChild(r); });
      ac.appendChild(list);
    } else {
      ac.appendChild(el('p', 'hint', 'Complete a few tasks and your patterns will show up here.'));
    }
    const aout = el('div', null); aout.id = 'aiOut'; aout.style.margin = '12px 0 0';
    const ab = el('button', 'btn block', '✨ Coach me'); ab.id = 'aiBtn';
    ab.addEventListener('click', () => aiCoach(aout, ins));
    ac.appendChild(ab);
    const askWrap = el('div', 'ask-wrap');
    const inp = el('input'); inp.id = 'aiAsk'; inp.className = 'ask-input'; inp.placeholder = 'Ask about your habits…'; inp.setAttribute('aria-label', 'Ask about your habits');
    inp.addEventListener('keydown', (ev) => { if (ev.key === 'Enter') aiAsk(aout, inp.value, ins); });
    const askBtn = el('button', 'btn ghost', 'Ask'); askBtn.id = 'aiAskBtn';
    askBtn.addEventListener('click', () => aiAsk(aout, inp.value, ins));
    askWrap.appendChild(inp); askWrap.appendChild(askBtn);
    ac.appendChild(askWrap);
    ac.appendChild(aout);
    v.appendChild(ac);
  }
...
function renderSettings() {
  ...
  v.appendChild(el('div', 'section-title', 'Feedback'));
  const fg = el('div', 'set-group');
  const fr = el('div', 'set-row');
  fr.innerHTML = '<div class="lblwrap"><div class="lbl">Send feedback</div><div class="sub">Tell us what\u2019s working, or what isn\u2019t</div></div>';
  const fbBtn = el('button', 'btn ghost', 'Write'); fbBtn.id = 'feedbackBtn'; fbBtn.addEventListener('click', openFeedback);
  fr.appendChild(fbBtn); fg.appendChild(fr); v.appendChild(fg);
...
function openEditor(id) {
  ...
  if (S.settings.aiEnabled) {
    ...
    const cl = el('button', 'btn ghost', '✨ Refine'); cl.id = 'ef-refine'; cl.type = 'button'; cl.addEventListener('click', aiRefine);
    ...
    const aiHint = el('div', 'ai-hint', 'Refine rewrites your task clearer with a when/where cue · Idea suggests what to actually do');
    fTitle.appendChild(aiHint);
...
function init() {
  ...
  if (S.settings.gcalConnected) gcalCacheLoad();
  renderHeader();
  setView('today');
  scheduleReminders();
...
let gcalNeedsAuth = false;
const GCAL_CACHE_KEY = 'tally.gcal.v1';
let gcalEventsByDay = {};
function calCacheSet(key, evs) { gcalEventsByDay[key] = evs; }
function gcalCacheLoad() {
  try { const o = JSON.parse(localStorage.getItem(GCAL_CACHE_KEY) || '{}'); if (o && o.days) gcalEventsByDay = o.days; } catch (e) {}
}
function gcalCacheSave() {
  try {
    const t0 = new Date(todayKey() + 'T12:00:00').getTime();
    const days = {};
    Object.keys(gcalEventsByDay).forEach(k => {
      const dt = new Date(k + 'T12:00:00').getTime();
      if (!isNaN(dt) && Math.abs(dt - t0) <= 31 * 86400000) days[k] = gcalEventsByDay[k];
    });
    localStorage.setItem(GCAL_CACHE_KEY, JSON.stringify({ days, ts: Date.now() }));
  } catch (e) {}
}
function loadGIS() {
...
async function gcalFetchDay(key) {
  if (!S.settings.gcalConnected) return;
  const wasUndef = gcalEventsByDay[key] === undefined;
  gcalEventsByDay[key] = gcalEventsByDay[key] || [];
  const tok = await gcalEnsureToken();
  if (!tok) { gcalNeedsAuth = true; if (wasUndef) delete gcalEventsByDay[key]; if (currentView === 'today') renderToday(); return; }
  gcalNeedsAuth = false;
...
function openAIConsent() {
  ...
  o.innerHTML = '<div class="celebrate-card" role="dialog" aria-modal="true">'
    ...
    + '<p class="celebrate-body">Get one gentle, specific tip drawn from your week. Tap “Coach me” on Stats whenever you like, or ask it anything about your habits.</p>'
...
const COACH_KEY = 'tally.coach.v1';
function coachSig() {
  const p = aiPayload();
  return p.weekPct + '|' + p.streak + '|' + p.tasks.map(t => t.title + ':' + t.progress).join(',') + '|' + computeInsights().join('|');
}
function coachCacheGet() { try { return JSON.parse(localStorage.getItem(COACH_KEY) || 'null'); } catch (e) { return null; } }
function renderCoachCard(out, text, cached) {
  out.innerHTML = '<div class="ai-card"><div class="ai-text"></div><div class="ai-badge">✨ AI-generated — a coach, not gospel' + (cached ? ' · <a href="#" class="ai-refresh">refresh</a>' : '') + '</div></div>';
  out.querySelector('.ai-text').textContent = text;
  const rf = out.querySelector('.ai-refresh');
  if (rf) rf.addEventListener('click', (e) => { e.preventDefault(); aiCoach(out, computeInsights(), true); });
}
async function aiCoach(out, insights, force) {
  if (!out) return;
  const sig = coachSig();
  const cache = coachCacheGet();
  if (!force && cache && cache.sig === sig && cache.text) { renderCoachCard(out, cache.text, true); return; }
  if (!AI_WORKER_URL) { out.innerHTML = '<div class="about" style="padding:0">Smart suggestions aren’t set up yet.</div>'; return; }
  out.innerHTML = '<div class="ai-loading">Reading your week…</div>';
  const lastTip = (cache && cache.text) ? cache.text : '';
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.assign({ mode: 'coach', insights: (insights || []).join(' '), lastTip: lastTip }, aiPayload())) });
    const data = await res.json();
    const text = data && (data.coach || data.suggestion);
    if (res.ok && text) {
      const clean = aiClean(text);
      try { localStorage.setItem(COACH_KEY, JSON.stringify({ sig: sig, text: clean })); } catch (e) {}
      renderCoachCard(out, clean, false);
    } else { out.innerHTML = '<div class="about" style="padding:0">Couldn’t reach your coach right now — try again in a moment.</div>'; }
  } catch (e) { out.innerHTML = '<div class="about" style="padding:0">Couldn’t reach your coach right now — try again in a moment.</div>'; }
}
async function aiAsk(out, question, insights) {
  if (!out) return;
  const q = (question || '').trim();
  if (!q) { const i = $('#aiAsk'); if (i) i.focus(); return; }
  if (!AI_WORKER_URL) { out.innerHTML = '<div class="about" style="padding:0">Smart suggestions aren’t set up yet.</div>'; return; }
  out.innerHTML = '<div class="ai-loading">Thinking…</div>';
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.assign({ mode: 'ask', question: q, insights: (insights || []).join(' ') }, aiPayload())) });
    const data = await res.json();
    const text = data && (data.answer || data.suggestion);
    if (res.ok && text) {
      out.innerHTML = '<div class="ai-card"><div class="ai-q"></div><div class="ai-text"></div><div class="ai-badge">✨ AI-generated — based on your data</div></div>';
      out.querySelector('.ai-q').textContent = q;
      out.querySelector('.ai-text').textContent = aiClean(text);
    } else { out.innerHTML = '<div class="about" style="padding:0">Couldn’t answer right now — try again.</div>'; }
  } catch (e) { out.innerHTML = '<div class="about" style="padding:0">Couldn’t answer right now — try again.</div>'; }
}
const DOW_FULL = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
function completionsByWeekday(weeks) {
  const counts = [0, 0, 0, 0, 0, 0, 0];
  const cutoff = mondayOf(new Date()); cutoff.setDate(cutoff.getDate() - (weeks - 1) * 7);
  const ck = dateKey(cutoff);
  allCompletionDays().forEach(d => { if (d >= ck) counts[(new Date(d + 'T12:00:00').getDay() + 6) % 7]++; });
  return counts;
}
function weekendDropoff() {
  const cutoff = mondayOf(new Date()); cutoff.setDate(cutoff.getDate() - 7 * 8);
  const ck = dateKey(cutoff);
  let wk = 0, we = 0;
  allCompletionDays().forEach(d => {
    if (d < ck) return;
    const dow = (new Date(d + 'T12:00:00').getDay() + 6) % 7;
    if (dow >= 5) we++; else wk++;
  });
  if (wk + we < 8) return null;
  const wkPer = wk / 5, wePer = we / 2;
  if (wePer < wkPer * 0.55) return '📉 Completions dip on weekends — lighter Sat/Sun goals may help.';
  return null;
}
function computeInsights() {
  const out = [];
  if (allCompletionDays().length < 3) return out;
  const wd = completionsByWeekday(8);
  const mx = Math.max(...wd);
  if (mx > 0) out.push('🗓️ Your strongest day is ' + DOW_FULL[wd.indexOf(mx)] + '.');
  const wts = weeklyTotals(2);
  const last = wts[0].count, now = wts[1].count;
  if (last || now) out.push(now > last ? '📈 You’re ahead of last week (' + now + ' vs ' + last + ').' : now < last ? '🌱 A little behind last week (' + now + ' vs ' + last + ') — easy to close.' : '⚖️ Holding steady with last week (' + now + ').');
  const cs = currentStreak();
  if (cs >= 3 && cs === longestStreak()) out.push('🏆 ' + cs + ' days — your longest streak yet!');
  const active = S.tasks.filter(t => !t.archived && (t.type === 'weekly' || t.type === 'daily' || t.type === 'weekdays'));
  let best = null, bestRatio = -1;
  active.forEach(t => {
    const ratio = t.type === 'weekly' ? (t.target ? compsThisWeek(t) / t.target : 0) : daysDoneThisWeek(t) / 7;
    if (ratio > bestRatio) { bestRatio = ratio; best = t; }
  });
  if (best && bestRatio > 0) out.push('🔥 Most consistent this week: ' + best.title + '.');
  const wek = weekendDropoff();
  if (wek) out.push(wek);
  return out.slice(0, 4);
}

async function aiRefine() {
  const titleEl = $('#ef-title'); if (!titleEl) return;
  const t = (titleEl.value || '').trim();
  if (!t) { titleEl.focus(); return; }
  if (!AI_WORKER_URL) { toast('Smart suggestions aren’t set up yet.'); return; }
  const btn = $('#ef-refine'); const label = btn ? btn.textContent : '';
  if (btn) { btn.disabled = true; btn.textContent = 'Refining…'; }
  const anchors = S.tasks.filter(x => !x.archived && (x.type === 'daily' || x.type === 'weekdays')).map(x => x.title).slice(0, 10);
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'clarify', title: t, anchors: anchors }) });
    const data = await res.json();
    const c = data && (data.clarified || data.suggestion);
    if (res.ok && c) { titleEl.value = c; if (draft) draft.title = c; toast('✨ Refined — tweak it however you like'); }
    else toast('Couldn’t refine right now — try again.');
  } catch (e) { toast('Couldn’t refine right now — try again.'); }
...
function openFeedback() {
  ...
  o.innerHTML = '<div class="celebrate-card" role="dialog" aria-modal="true">'
    + '<div class="celebrate-emoji">\ud83d\udcac</div>'
    + '<h2 class="celebrate-title">Send feedback</h2>'
    + '<p class="celebrate-body">What do you love, what\u2019s confusing, what\u2019s missing? It opens your email, pre-filled.</p>'
    + '<textarea id="fbText" class="fb-text" placeholder="Your thoughts\u2026"></textarea>'
    + '<div class="sheet-actions"><button class="btn ghost" id="fbCancel">Cancel</button><button class="btn" id="fbSend">Send</button></div>'
    + '</div>';
  o.hidden = false;
  setTimeout(() => { const t = $('#fbText'); if (t) t.focus(); }, 50);
  $('#fbCancel').addEventListener('click', closeCelebrate);
  $('#fbSend').addEventListener('click', () => {
    const txt = ($('#fbText').value || '').trim();
    const body = encodeURIComponent(txt + '\n\n— sent from Tally');
    const subject = encodeURIComponent('Tally feedback');
    closeCelebrate();
    window.location.href = 'mailto:gerasimosdap@gmail.com?subject=' + subject + '&body=' + body;
  });
}

function aiClean(s) {
  return String(s || '').replace(/\*\*/g, '').replace(/^\s*[\*\-]\s+/gm, '• ').replace(/`/g, '').trim();
}
async function aiAssist() {
  ...
  out.innerHTML = '<div class="ai-loading">Thinking\u2026</div>';
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'assist', title: t }) });
    const data = await res.json();
    const text = data && (data.assist || data.suggestion);
    if (res.ok && text) {
