...
async function aiCoach(out, insights) {
  if (!out) return;
  ...
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.assign({ mode: 'coach', insights: (insights || []).join(' ') }, aiPayload())) });
    const data = await res.json();
    const text = data && (data.coach || data.suggestion);
    if (res.ok && text) {
      out.innerHTML = '<div class="ai-card"><div class="ai-text"></div><div class="ai-badge">✨ AI-generated — a coach, not gospel</div></div>';
