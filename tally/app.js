...
async function aiClarify() {
  ...
  const btn = $('#ef-clarify'); const label = btn ? btn.textContent : '';
  if (btn) { btn.disabled = true; btn.textContent = 'Sharpening…'; }
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'clarify', title: t }) });
    const data = await res.json();
    const c = data && (data.clarified || data.suggestion);
    if (res.ok && c) { titleEl.value = c; if (draft) draft.title = c; toast('✨ Sharpened — tweak it however you like'); }
    else toast('Couldn’t sharpen right now — try again.');
  } catch (e) { toast('Couldn’t sharpen right now — try again.'); }
