...
/* ---------------- Proactive check-in (AI, opt-in) ---------------- */
function laggingGoal() {
  if (!S.settings.aiEnabled) return null;
  const dim = dowMon(new Date());          // 0 = Mon ... 6 = Sun
  if (dim < 2) return null;                 // only nudge from Wednesday on
  const wk = weekKey();
  for (const t of S.tasks) {
    if (t.archived || t.type !== 'weekly') continue;
    const done = compsThisWeek(t);
    if (done >= t.target) continue;
    const daysLeft = 7 - dim;               // includes today
    const need = t.target - done;
    const behind = need > daysLeft || (done === 0) || (done / t.target) < ((dim + 1) / 7) * 0.5;
    if (!behind) continue;
    if (S.settings.proactiveDismiss === wk + '|' + t.id) continue;
    return t;
  }
  return null;
}
function renderProactive(v) {
  const t = laggingGoal();
  if (!t) return;
  const card = el('div', 'proactive');
  card.innerHTML = '<div class="pro-row"><span class="pro-ico">\ud83e\udd1d</span><div class="pro-text">A little behind on <b>' + escapeHtml(t.title) + '</b> this week \u2014 want a hand?</div></div>';
  const acts = el('div', 'pro-acts');
  const yes = el('button', 'btn', 'Yes, help'); yes.addEventListener('click', () => aiHelp(t, card));
  const no = el('button', 'btn ghost', 'Not now'); no.addEventListener('click', () => { S.settings.proactiveDismiss = weekKey() + '|' + t.id; save(); renderToday(); });
...
function aiHelp(t, card) { askHelpBlocker(t, card); }
function askHelpBlocker(t, card) {
  card.innerHTML = '<div class="pro-text" style="margin-bottom:10px">What’s getting in the way of <b>' + escapeHtml(t.title) + '</b>?</div>';
  const chips = el('div', 'help-chips');
  ['Time', 'Energy', 'Not sure how', 'Motivation'].forEach(b => {
    const ch = el('button', 'chip', b); ch.type = 'button';
    ch.addEventListener('click', () => askHelpTime(t, card, b));
    chips.appendChild(ch);
  });
  card.appendChild(chips);
}
function askHelpTime(t, card, blocker) {
  card.innerHTML = '<div class="pro-text" style="margin-bottom:10px">How much time do you realistically have today?</div>';
  const chips = el('div', 'help-chips');
  ['2 minutes', '10 minutes', '20+ minutes'].forEach(tm => {
    const ch = el('button', 'chip', tm); ch.type = 'button';
    ch.addEventListener('click', () => fetchHelp(t, card, blocker, tm));
    chips.appendChild(ch);
  });
  card.appendChild(chips);
}
async function fetchHelp(t, card, blocker, time) {
  if (!AI_WORKER_URL) { card.innerHTML = '<div class="about" style="padding:0">Smart suggestions aren’t set up yet.</div>'; return; }
  card.innerHTML = '<div class="ai-loading">Thinking…</div>';
  const progress = compsThisWeek(t) + '/' + t.target + ' this week. Their main blocker: ' + blocker + '. Time available today: ' + time + '.';
  try {
    const res = await fetch(AI_WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'help', title: t.title, progress }) });
    const data = await res.json();
    const text = data && (data.help || data.suggestion);
    if (res.ok && text) {
      card.innerHTML = '<div class="ai-card"><div class="ai-text"></div><div class="ai-badge">✨ AI-generated — a suggestion, not gospel</div></div>';
      card.querySelector('.ai-text').textContent = aiClean(text);
      S.settings.proactiveDismiss = weekKey() + '|' + t.id; save();
    } else { card.innerHTML = '<div class="about" style="padding:0">Couldn’t fetch a tip right now — try again later.</div>'; }
  } catch (e) { card.innerHTML = '<div class="about" style="padding:0">Couldn’t fetch a tip right now — try again later.</div>'; }
}
